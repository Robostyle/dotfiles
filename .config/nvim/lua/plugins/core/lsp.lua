...
local function extend_capabilities(servers)
  ...
  for server, server_opts in pairs(servers) do
    local extended_capabilities =
      vim.tbl_deep_extend('force', client_capabilities, server_opts.capabilities or {})
...
local function register_lsp_servers(servers)
  for server, server_opts in pairs(servers) do
    if server_opts.enabled == false then
      ...
      if
        vim.lsp.config[server].filetypes == nil
        and not vim.tbl_contains({ 'copilot' }, server)
      then
      ...
      if not vim.lsp.config[server].root_dir and not vim.lsp.config[server].root_markers then
        vim.notify(
          'No root_dir or root_markers specified for LSP server: ' .. server,
          vim.log.levels.ERROR
        )
      end
      if vim.lsp.config[server].root_dir and vim.lsp.config[server].root_markers then
        vim.notify(
          'Both root_dir and root_markers specified for LSP server (root_dir will be used): '
            .. server,
          vim.log.levels.ERROR
        )
      end
    ...
    callback = function(args)
      ...
      if client then
        ...
        if client:supports_method('workspace/diagnostic', args.buf) then
          ...
          vim.notify_once(
            'Setting up workspace diagnostics for ' .. client.name,
            vim.log.levels.WARN
          )
        ...
        if client:supports_method('textDocument/foldingRange', args.buf) then
          ...
          require('config.options').lsp_foldexpr(win)
        end
      ...
      -- set up keymaps
      require('config.keymaps').setup_lsp_autocmd_keymaps(args.buf)
    end,
  })
end
...
local function register_lsp_commands()
  ...
  vim.api.nvim_create_user_command('LspRestart', function(args)
    local filter = nil
    if args.args ~= '' then filter = { name = args.args } end
    ...
    for _, client in ipairs(clients) do
      ...
      vim.diagnostic.reset(
        vim.api.nvim_create_namespace('vim.lsp.' .. name .. '.' .. client.id)
      )

      -- Clear codelens for this client
      if vim.lsp.codelens then vim.lsp.codelens.enable(false, { client_id = client.id }) end

      vim.lsp.enable(name, false)
      if args.bang then client:stop(true) end
    end

    if #clients == 0 and args.args ~= '' then table.insert(client_names, args.args) end
    ...
    complete = function()
      ...
      local names = {}
      for _, client in ipairs(clients) do
        if not vim.tbl_contains(names, client.name) then table.insert(names, client.name) end
      end
      return names
    end,
